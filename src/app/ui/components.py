...
def create_candidate_card(candidate_data, on_summary_click_callback):
    ...
    keyword_controls = [
        ft.Text(f"{i}. {kw}: {count} occurences", color="#444444", size=12) 
        for i, (kw, count) in enumerate(candidate_data["matched_keywords"].items(), 1)
    ]

    card_content = ft.Column([
        ft.Row([
            ft.Text(candidate_data["name"], weight=ft.FontWeight.BOLD, color="black", size=18),
            ft.Container(
                content=ft.Text(f"{candidate_data['matches']} Matched", color="#1e1e2f", size=10, weight=ft.FontWeight.BOLD),
